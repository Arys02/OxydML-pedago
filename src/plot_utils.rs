...
pub(crate) fn plot_data(data_src: String, shape_len_x: [f64; 2], shape_len_y: [f64; 2], is_classif: bool){
    ...
    let (x1 , y1) = x2.view().split_at(Axis(1), 1);
    ...
    linear_r._fit(x1_concatenated, y1.into_owned(), 2000, 0.01, false);
...
fn get_csv2_f64(src : &String) -> Result<ArrayBase<OwnedRepr<f64>, Ix2>, Box<dyn Error>> {
...
fn get_csv2(src : &String) -> Result<Array2<f32>, Box<dyn Error>>{
    let mut rdr = csv::ReaderBuilder::new().has_headers(false).from_path(src)?;
